...
class VertexData {
	...
	private static function __init__() {
		untyped Object.defineProperties(VertexData.prototype, {
			"premultipliedAlpha": {
				get: untyped __js__("function () { return this.get_premultipliedAlpha (); }"),
				set: untyped __js__("function (v) { return this.set_premultipliedAlpha (v); }")
			},
			"numVertices": {
				get: untyped __js__("function () { return this.get_numVertices (); }"),
				set: untyped __js__("function (v) { return this.set_numVertices (v); }")
			},
	...
	public function new(format:Dynamic = null, initialCapacity:Int = 32) {
		...
		else if (#if (haxe_ver < 4.2) Std.is #else Std.isOfType #end (format, VertexDataFormat))
			_format = format;
		else if (#if (haxe_ver < 4.2) Std.is #else Std.isOfType #end (format, String))
			_format = VertexDataFormat.fromString(Std.string(format));
		else
	...
	private function copyAttributeTo_internal(target:VertexData, targetVertexID:Int, matrix:Matrix, sourceAttribute:VertexDataAttribute,
			targetAttribute:VertexDataAttribute, vertexID:Int, numVertices:Int):Void {
	...
	/** Writes the given coordinates to the specified vertex and attribute. */
	public function setPoint4D(vertexID:Int, attrName:String, x:Float, y:Float, z:Float, w:Float = 1.0):Void {
		if (_numVertices < vertexID + 1)
			numVertices = vertexID + 1;
	 ...
	 *  instead of creating a new object. To use all vertices for the calculation, set
	 *  'numVertices' to '-1'.</p> */
	public function getBoundsProjected(attrName:String, matrix:Matrix3D, camPos:Vector3D, vertexID:Int = 0, numVertices:Int = -1,
	...
	public function translatePoints(attrName:String, deltaX:Float, deltaY:Float, vertexID:Int = 0, numVertices:Int = -1):Void {
		if (numVertices < 0 || vertexID + numVertices > _numVertices)
			numVertices = _numVertices - vertexID;
	...
	public function scaleAlphas(attrName:String, factor:Float, vertexID:Int = 0, numVertices:Int = -1):Void {
		if (factor == 1.0)
			return;
		if (numVertices < 0 || vertexID + numVertices > _numVertices)
			numVertices = _numVertices - vertexID;
	...
	public function colorize(attrName:String = "color", color:UInt = 0xffffff, alpha:Float = 1.0, vertexID:Int = 0, numVertices:Int = -1):Void {
		if (numVertices < 0 || vertexID + numVertices > _numVertices)
			numVertices = _numVertices - vertexID;
	...
	public function createVertexBuffer(upload:Bool = false, bufferUsage:String = "staticDraw"):VertexBuffer3D {
		...
		var buffer:VertexBuffer3D = context.createVertexBuffer(_numVertices, Std.int(_vertexSize / 4), bufferUsage);
